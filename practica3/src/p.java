...
public class p {
    ...
    private static void resolver(GeneralTree<Integer> arbol, List<Integer> camAct, List<Integer> camMax, max m,
            int valorTotal, int nivel) {
        ...
        if (!arbol.isLeaf()) {
            for (GeneralTree<Integer> h : arbol.getChildren()) {
                resolver(h, camAct, camMax, m, valorTotal, nivel + 1);
            }
        } else if (valorTotal > m.getMax()) {
            m.setMax(valorTotal);
            valorTotal = 0;
