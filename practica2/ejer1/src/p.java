...
public class p {
    ...
    public p(BinaryTree<Integer> ab) {
        this.ab = ab;
    }
    ...
    private int contarUnicoHijo(BinaryTree<Integer> arb) {
        int cant = 0;
        if (arb.hasLeftChild())
            cant += contarUnicoHijo(arb.getLeftChild());
        if (arb.hasRightChild())
            cant += contarUnicoHijo(arb.getRightChild());
    ...
    private boolean isLeftTree(BinaryTree<Integer> arb) {
        ...
        int ramaDer = -1;
        if (arb.hasLeftChild())
            ramaIzq = contarUnicoHijo(arb.getLeftChild());
        if (arb.hasRightChild())
            ramaDer = contarUnicoHijo(arb.getRightChild());
