...
public class mapac {
    ...
    public mapac(Graph<String> mapa) {
    ...
    // La idea es devolver el primer camino encontrado, y recorrer con un while. No
    // seria necesario devolver cada camino que se encuentre y usar un for.
    ...
    private boolean devolverCamino(Vertex<String> origen, Vertex<String> destino, List<String> camino,
        ...
        if (!encontre) {
            camino.remove(camino.size() - 1); // marcas[origen.getPosition()] = false; Cuando haya encontrado el camino
                                              // hacia destino, deja marcadas las posiciones del vector de marcas
        }
        marcas[origen.getPosition()] = false; // Deja desmarcadas todas las posiciones del vector de marcas: no es
                                              // necesario desmarcar en este ejemplo.
        return encontre;
    }
    ...
    public List<String> caminoMasCorto(String ciudad1, String ciudad2) {
        ...
        if (!this.mapaCiudades.isEmpty()) {
            ...
            Vertex destino = this.mapaCiudades.search(ciudad2);
            if (origen != null && destino != null) {
                caminoMasCorto(origen, destino, camino, new LinkedList<String>(),
                        new boolean[this.getMapaCiudades().getSize()], 0, Integer.MAX_VALUE);
            }
        }
        return camino;
    }
    ...
    private int caminoMasCorto(Vertex<String> origen, Vertex<String> destino, List<String> caminoMinimo,
            List<String> caminoAct, boolean[] marcas, int total, int min) {
        marcas[origen.getPosition()] = true;
    ...
    public List<String> caminoSinCargarCombustible(String ciudad1, String ciudad2, int tanqueAuto) {
        ...
        if (!this.mapaCiudades.isEmpty()) {
            ...
            Vertex destino = this.mapaCiudades.search(ciudad2);
            if (origen != null && destino != null) {
                caminoSinCargarCombustible(origen, destino, camino, new boolean[this.getMapaCiudades().getSize()],
                        tanqueAuto);
            }
        }
        return camino;
        ...
        if (origen == destino) {
            ...
            while (it.hasNext() && !encontre) {
                ...
                if (!marcas[j] && tanqueAuto - v.getWeight() > 0) {
                    encontre = caminoSinCargarCombustible(v.getTarget(), destino, camino, marcas,
                            tanqueAuto - v.getWeight());
                }
            }
        }
        if (!encontre) {
    ...
    public List<String> caminoConMenorCargaCombustible(String ciudad1, String ciudad2, int tanqueAuto) {
        ...
        if (!this.mapaCiudades.isEmpty()) {
            ...
            Vertex destino = this.mapaCiudades.search(ciudad2);
            if (origen != null && destino != null) {
                caminoConMenorCargaCombustible(origen, destino, camino, new LinkedList<String>(),
    ...
    private int caminoConMenorCargaCombustible(Vertex<String> origen, Vertex<String> destino, List<String> caminoMinimo,
            List<String> caminoAct, boolean[] marcas, int tanqueActual, int tanque, int recargas, int recargasMin) {
        marcas[origen.getPosition()] = true;
        caminoAct.add(origen.getData());
            ...
            while (it.hasNext() && recargas < recargasMin) {
                ...
                if (!marcas[j]) {
                    if (tanqueActual >= distancia) {
                        recargasMin = caminoConMenorCargaCombustible(v.getTarget(), destino, caminoMinimo, caminoAct,
                                marcas, tanqueActual - distancia, tanque, recargas, recargasMin);
                    } else if (tanque >= distancia) {
                        recargasMin = caminoConMenorCargaCombustible(v.getTarget(), destino, caminoMinimo, caminoAct,
                                marcas, tanque - distancia, tanque, recargas + 1, recargasMin);
                    }
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        mapac mapa = new mapac(mapaCiudades);
